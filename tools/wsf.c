...
static bool wsf_lib_path(char *buf, size_t len) {
	...
	if (override != NULL && override[0] != '\0') {
		...
		/*
		 * This value is persisted into environment.d as part of
		 * LD_PRELOAD, so it must be an absolute path with no separator
		 * or control characters — a relative path would be resolved
		 * against each session process's cwd (a classic preload footgun)
		 * and a ':' or newline would corrupt the env file.
		 */
		if (override[0] != '/') {
			fprintf(stderr,
				"WSF_LIB_PATH must be an absolute path: %s\n", override);
			return false;
		}
		for (i = 0; override[i] != '\0'; i++) {
			unsigned char c = (unsigned char) override[i];

			/* c < 0x20 already covers newline/CR/control chars. */
			if (c == ':' || c < 0x20) {
				fprintf(stderr,
					"WSF_LIB_PATH contains an invalid character.\n");
				return false;
			}
		}
...
static bool wsf_write_env_file(
	...
	/*
	 * Atomic write: a crash/power-loss mid-write to environment.d would
	 * otherwise leave a truncated LD_PRELOAD= line that ld.so injects into
	 * every process of the next session. mkstemp in the same directory +
	 * rename gives an all-or-nothing replacement.
	 */
	written = snprintf(tmp_path, sizeof(tmp_path), "%s.tmp.XXXXXX", env_path);
	...
	if (fprintf(file, "# Generated by wsf\n") < 0 ||
...
static int wsf_cmd_enable(void) {
	char env_path[512];
	char lib_path[512];
	char preload_value[WSF_ENV_VALUE_MAX];
	...
	/*
	 * Preserve any foreign LD_PRELOAD entries the user already has, the
	 * same way `wsf repair` does — otherwise enabling WSF would silently
	 * drop a co-installed preload from the session env file.
	 */
	if (!wsf_repaired_preload_value(existing, lib_path,
		preload_value, sizeof(preload_value))) {
		fprintf(stderr, "Failed to compose LD_PRELOAD value.\n");
		return 1;
	}

	if (!wsf_write_env_file(env_path, preload_value)) {
