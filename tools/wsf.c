...
static bool wsf_env_file_preload_matches(
	...
	if (env_path == NULL || lib_path == NULL) {
		return false;
	}
	...
	while (fgets(line, (int) sizeof(line), file) != NULL) {
		...
		if (wsf_preload_list_contains(cursor + key_len, lib_path)) {
			fclose(file);
			return true;
		}
	}
...
static bool wsf_write_env_file(const char *env_path, const char *lib_path) {
	FILE *file = NULL;
...
static bool wsf_repair_preload_setup(
	...
	if (!env_matches) {
		...
		printf("repair: environment.d already points at WSF\n");
	}
...
static int wsf_cmd_repair(void) {
	...
	repaired = wsf_repair_preload_setup(env_path, lib_path, true);
	if (!repaired) {
		return 1;
	}
	...
	if (!runtime.user_manager_matches) {
		printf("repair: environment file is %s, but systemd --user still does not expose WSF.\n",
	...
	printf("repair: preload setup is ready for the next GNOME session.\n");
...
static bool wsf_systemd_user_env_value(
	...
	while (fgets(raw, (int) sizeof(raw), pipe) != NULL) {
		raw[strcspn(raw, "\n")] = '\0';
